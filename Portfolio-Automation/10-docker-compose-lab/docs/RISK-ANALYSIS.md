...
| ------- | ---- | ----- | ------ | -------- | ------ | -------------------- | --- |
| DL-R01 | `/health` reports the database as up without checking it. | High | High | Critical | **Observed → fixed** | `backend/src/main.py:39-45` — `check_db_connection` only logs `"Mock checking DB connection"` and returns `True`. | T12 (done) |
| DL-R02 | The Docker healthcheck of `fastapi-gateway` can never fail. | High | High | Critical | **Observed → fixed** | `backend/src/main.py:102-104` returns HTTP 200 with `"status": "degraded"`; `docker-compose.yml:130` uses `curl -f`, which only fails on HTTP ≥ 400. Traefik keeps routing to a broken gateway. | T12 (done) |
| DL-R03 | The API signs tokens with a public default secret. | High | High | Critical | **Observed → fixed** | `docker-compose.yml:110` passes `JWT_SECRET_FILE`, but `backend/src/config.py:15` has no such field and defaults `JWT_SECRET` to `"insecure-default-secret-change-in-production"`. The Docker secret is mounted and ignored. | T12 (done) |
| DL-R04 | Database role passwords are committed in plaintext. | High | High | Critical | **Observed → fixed** | `init-db.sql:8,13,18` and `docker-compose.yml:108,146,197,280` (`fastapi_secure_pass`, `n8n_secure_pass`, `grafana_secure_pass`). Anyone with repo access knows them; `secrets/` only protects the superuser. | T11 (done) |
| DL-R05 | The API starts without a real database configured. | Medium | Medium | High | **Observed → fixed** | `backend/src/config.py:12` defaults `DATABASE_URL` to `sqlite+aiosqlite:///./test.db`; `aiosqlite` is not in `requirements.txt`, so a missing variable fails late and confusingly instead of at startup. | T12 (done) |
...
| DL-R09 | Making settings mandatory breaks the Celery worker/beat. | Medium | High | High | Mitigated | `backend/src/celery_app.py` imports `Settings`; `celery-beat` has no `DATABASE_URL` and neither worker has a JWT secret. Requirements must be enforced only at API startup (`require_api_settings`), never in `Settings`. | T12 (done) |
| DL-R10 | Nothing proves the backend works. | High | Medium | High | **Observed → fixed** | `backend/` has no tests and is not in CI. | T12 (done) |
